...
const TravelStoryCard = ({
  ...
  return (
    <div className="border rounded-lg overflow-hidden bg-white hover:shadow-lg  hover:shadow-slate-200 trasition-all ease-in-out relative cursor-pointer">
      <img
        src={imageUrl}
        alt={title}
      ...
      <div className="p-4" onClick={onClick}>
        ...
        <div className="inline-flex items-center gap-2 text-[13px] text-cyan-600 bg-cyan-200/40 rounded mt-3 px-2 py-1">
          ...
          {visitedLocation.map((item, index) => {
            return visitedLocation.length === index + 1
              ? `${item}`
              : `${item}, `;
          })}
        </div>
